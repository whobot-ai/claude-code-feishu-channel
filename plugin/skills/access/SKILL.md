...
7. `mkdir -p ~/.claude/channels/feishu/approved` then write
   ...
   file contents. The channel server polls this dir and sends "配对成功".
...
- `ackReaction`: string (Feishu emoji_type like THUMBSUP) or `""` to disable
- `textChunkLimit`: number (max chars per outbound message)
...
- Sender IDs are Feishu **open_id** values (like `ou_xxx`). They are
  app-specific and stable. Chat IDs are `oc_xxx` for group chats.
  Don't confuse sender open_id with chat_id.
